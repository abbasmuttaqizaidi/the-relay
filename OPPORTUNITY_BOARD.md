...
- **Search & Advanced Filters:**
  ...
  - Industry multi-select dropdown (SaaS, AI & Automation, D2C, Logistics, Legal, etc.).
  - Location search filter.
  - Expiration filter (e.g. Active Only, Expiring within 7/14/30 days).
- **Reciprocity Score Indicator:**
  - Displays business reciprocity health score to incentivize equitable exchange behavior.
- **Save / Bookmark Toggle:**
  - 1-click save/unsave with optimistic UI updates.
- **Guided Product Tour (`driver.js`):**
  - Interactive walkthrough for first-time onboarding users.

### 5.2 Opportunity Card Anatomy
Each opportunity card is rendered in the pure monochrome design system:
1. **Header:** Sequential Opportunity ID (`RY-0042`), Category badge, and Expiration timeline.
...
### 5.3 Express Interest & Pitch Submission
When a business expresses interest:
1. Opens the **Express Interest Modal**.
2. Shows summary of the opportunity terms.
3. Prompts the requester to formulate their value exchange proposal pitch.
4. On submit, calls `expressInterest`, creating an `Interest` record in `status: "pending"` and alerting the Opportunity Owner via the in-app notification system.
