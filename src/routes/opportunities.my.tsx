    ...
    <div className="min-h-screen bg-slate-50/50 text-slate-900 selection:bg-slate-900 selection:text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-6 pb-24">
            ...
            <div className="mb-8 border border-slate-200 bg-white p-5 rounded-[2px] flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs animate-momentum">
              <div className="space-y-1 text-left">
                ...
                <p className="text-xs text-slate-600">
                  You have <span className="font-bold">{savedCount}</span> saved opportunities. Review them after approval.
                </p>
              </div>
              <Link
          ...
          <div className="mb-8 border border-emerald-500/20 bg-emerald-50 p-5 rounded-[2px] flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs animate-momentum">
            <div className="space-y-1 text-left">
              ...
              <p className="text-xs text-emerald-700 leading-relaxed max-w-[80ch]">
                Your business has been approved. You currently have <span className="font-bold text-emerald-900">{savedCount}</span> Saved Opportunities.
              </p>
            </div>
            <Link
              to="/saved-opportunities"
        ...
        <div className="flex border-b border-slate-200 mb-6 font-mono text-xs uppercase tracking-wider font-bold">
          ...
          <button
            onClick={() => navigate({ to: "/opportunities/my", search: { tab: "pending-handshakes" } })}
            className={`py-3 px-6 border-b-2 transition-all cursor-pointer ${
          ...
          {loadingList ? (
                    ...
                    <div key={opp.id} className="p-4 space-y-3">
                      ...
                      <div className="flex items-center justify-between border-t border-slate-50 pt-2.5">
                        <div className="flex items-center gap-3">
                          <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-mono font-bold uppercase tracking-wider ${
                            opp.status === "active" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-slate-100 text-slate-500 border border-slate-200"
                          }`}>
                            {opp.status}
                          </span>
                          <span className="text-[10px] font-mono text-slate-500 font-bold">{opp.interestedCount} Operators</span>
                        </div>
            ...
            savedItems.length === 0 ? (
              <div className="py-20 text-center flex flex-col items-center justify-center space-y-4">
                ...
                <div className="space-y-1">
                  <h4 className="font-display font-extrabold text-sm text-slate-900 uppercase">No Saved Opportunities</h4>
                  <p className="text-slate-500 text-xs max-w-sm">
            ...
            pendingHandshakes.length === 0 ? (
              <div className="py-20 text-center flex flex-col items-center justify-center space-y-4">
                ...
                <div className="space-y-1">
                  <h4 className="font-display font-extrabold text-sm text-slate-900 uppercase">No Pending Handshakes</h4>
                  <p className="text-slate-500 text-xs max-w-sm">
                    Express interest in other opportunities. When introduction contexts are sent, they await approval here.
                  </p>
                </div>
              </div>
            ) : (
                  ...
                  <table className="w-full text-left border-collapse font-sans text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {pendingHandshakes.map((opp) => {
                        ...
                        const shouldHide = opp.hide_company_name && !isConnected && !isApproved;
                        const displayName = shouldHide ? "Confidential" : (opp.company || opp.business?.company_name || "Confidential");
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-4 px-6 font-mono text-slate-400">
                              {record?.requestedAt ? new Date(record.requestedAt).toLocaleDateString() : "-"}
                            </td>
                            <td className="py-4 px-6 text-right">
                              <button
                                onClick={(e) => {
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="flex w-[calc(100vw-1rem)] max-w-[680px] max-h-[88dvh] flex-col gap-0 overflow-hidden bg-white p-0 sm:p-0 text-left font-sans shadow-xl rounded-[4px] border border-slate-200">
          ...
          <form onSubmit={handleCreateSubmit} className="min-h-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden px-4 py-4 sm:px-4 sm:py-3">
            {/* Title */}
            <div className="space-y-1">
              <label className="text-[9px] font-mono font-bold uppercase tracking-wider sm:tracking-widest text-slate-500 flex flex-wrap items-center gap-1.5">
                <span className="flex flex-wrap items-center gap-1"><Tag className="w-3 h-3" /> Opportunity Title *</span>
            ...
            <div className="space-y-1">
              <label className="text-[9px] font-mono font-bold uppercase tracking-wider sm:tracking-widest text-slate-500 flex flex-wrap items-center gap-1.5">
                <span className="flex flex-wrap items-center gap-1"><Briefcase className="w-3 h-3" /> Exchange Category *</span>
            ...
            <div className="space-y-1">
              <label className="text-[9px] font-mono font-bold uppercase tracking-wider sm:tracking-widest text-slate-500 flex flex-wrap items-center gap-1.5 justify-between">
                <span className="flex flex-wrap items-center gap-1.5">
                  <span className="flex flex-wrap items-center gap-1"><Info className="w-3 h-3" /> Brief Description *</span>
            ...
            <div className="grid min-w-0 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-mono font-bold uppercase tracking-wider sm:tracking-widest text-slate-500 flex flex-wrap items-center gap-1.5">
                  <span className="flex flex-wrap items-center gap-1"><MapPin className="w-3 h-3" /> Location Target (Optional)</span>
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-mono font-bold uppercase tracking-wider sm:tracking-widest text-slate-500 flex flex-wrap items-center gap-1.5">
                  <span className="flex flex-wrap items-center gap-1"><Clock className="w-3 h-3" /> Expiry Period (Optional)</span>
            ...
            <div className="space-y-1">
              <label className="text-[9px] font-mono font-bold uppercase tracking-wider sm:tracking-widest text-slate-500 flex flex-wrap items-center gap-1.5">
                <span className="flex flex-wrap items-center gap-1"><CheckCircle2 className="w-3 h-3" /> What Can You Offer in Return? (Optional)</span>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="flex w-[calc(100vw-1rem)] max-w-[680px] max-h-[88dvh] flex-col gap-0 overflow-hidden bg-white p-0 sm:p-0 text-left font-sans shadow-xl rounded-[4px] border border-slate-200">
          ...
          <form onSubmit={handleEditSubmit} className="min-h-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden px-4 py-4 sm:px-4 sm:py-3">
            {/* Title */}
            <div className="space-y-1">
              <label className="text-[9px] font-mono font-bold uppercase tracking-wider sm:tracking-widest text-slate-500 flex flex-wrap items-center gap-1.5">
                <span className="flex flex-wrap items-center gap-1"><Tag className="w-3 h-3" /> Opportunity Title *</span>
            ...
            <div className="grid min-w-0 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-mono font-bold uppercase tracking-wider sm:tracking-widest text-slate-500 flex flex-wrap items-center gap-1.5">
                  <span className="flex flex-wrap items-center gap-1"><Clock className="w-3 h-3" /> Extend Expiry Period (Optional)</span>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="sm:max-w-xl bg-white border border-[#1f25301f] rounded-[4px] p-6 shadow-xl font-sans text-left">
          {selectedDetailOpp && (() => {
            ...
            const isExpired = selectedDetailOpp.expires_at ? new Date(selectedDetailOpp.expires_at) < new Date() : false;
            const isInactive = isClosed || isExpired;
            ...
            const displayName = shouldHide ? "Confidential" : (selectedDetailOpp.company || selectedDetailOpp.business?.company_name || "Confidential");
                ...
                <DialogHeader className="space-y-2.5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 border-b border-slate-100 pb-2">
                    ...
                    <span className="font-mono text-[9px] text-slate-400 font-semibold">
                      #{selectedDetailOpp.opportunity_number || selectedDetailOpp.id.substring(0, 8)}
                    </span>
                  </div>
                ...
                <div className="space-y-4 pt-2">
                  ...
                  {selectedDetailOpp.offer_text && (
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-600 font-sans">{selectedDetailOpp.offer_text}</p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-b border-slate-100 py-3.5">
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="text-xs text-slate-700 font-mono">{selectedDetailOpp.industry || selectedDetailOpp.business?.industry}</span>
                    </div>
                    <div className="space-y-1">
                      <span className="font-mono text-[9px] uppercase tracking-widest text-slate-400 font-bold block">Location</span>
                      <span className="text-xs text-slate-700 font-mono">{selectedDetailOpp.location || "Remote"}</span>
                    </div>
                    <div className="space-y-1">
                      <span className="font-mono text-[9px] uppercase tracking-widest text-slate-400 font-bold block">Expires</span>
                      <span className="text-xs text-slate-700 font-mono">
                        {selectedDetailOpp.expires_at ? new Date(selectedDetailOpp.expires_at).toLocaleDateString() : "Never"}
                      </span>
                    </div>
                  </div>
                  ...
                  {interestStatus === "accepted" && store[selectedDetailOpp.id]?.contact && (
                    <div className="border border-emerald-500/20 bg-emerald-500/5 p-4 space-y-2 rounded-[2px] shadow-sm animate-momentum">
                      ...
                      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                        <span className="font-display text-sm font-bold text-slate-900">{store[selectedDetailOpp.id]?.contact?.name}</span>
                        <span className="font-mono text-[10px] text-slate-400 font-bold">· {store[selectedDetailOpp.id]?.contact?.role}</span>
                      </div>
                      <a
